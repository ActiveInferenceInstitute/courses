# Module 03: Perception

> **Quick Navigation**: [Course Home](../README.md) | [Curriculum Home](../../README.md)

## Seeing is Believing... Or Is It?

Part of **Real Life Skills**.

## Contents

| File | Description |
| --- | --- |
| [module.md](./module.md) | Full lecture (Seeing is Believing... Or Is It?) |
| [questions.md](./questions.md) | 20 Study Questions |
| [practice_quiz.md](./practice_quiz.md) | Practice Quiz (MC + Short Answer) |
| [lab.md](./lab.md) | Lab: Seeing is Believing... Or Is It? |
| [dashboard.html](./dashboard.html) | Interactive Dashboard |

## Learning Goals

1. **Explain** perception as prediction
2. **Identify** prediction errors in illusions
3. **Describe** how the brain filters input

## Resources

- [Notation](../../resources/notation_table.md)
- [Glossary](../../resources/glossary.md)
