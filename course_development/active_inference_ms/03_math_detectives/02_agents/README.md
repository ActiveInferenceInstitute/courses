# Module 02: Agents

> **Quick Navigation**: [Course Home](../README.md) | [Curriculum Home](../../README.md)

## What Makes an Agent Tick?

Part of **Math Detectives**.

## Contents

| File | Description |
| --- | --- |
| [module.md](./module.md) | Full lecture (What Makes an Agent Tick?) |
| [questions.md](./questions.md) | 20 Study Questions |
| [practice_quiz.md](./practice_quiz.md) | Practice Quiz (MC + Short Answer) |
| [lab.md](./lab.md) | Lab: What Makes an Agent Tick? |
| [dashboard.html](./dashboard.html) | Interactive Dashboard |

## Learning Goals

1. **Distinguish** agents from objects
2. **Describe** an agent's internal model
3. **Explain** goal-directed behavior

## Resources

- [Notation](../../resources/notation_table.md)
- [Glossary](../../resources/glossary.md)
