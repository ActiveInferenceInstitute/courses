# Station: Action (Control & Estimation)
...
- **Topics**: LQR, LQG, MPC, active inference control derivation, Active Inference equivalences
...
1. Present action through rigorous mathematical formulations with explicit equations and derivations.
2. Show the formal relationship between classical action approaches and Active Inference, including conditions under which they are equivalent.
3. Use standard control theory notation (A, B, C, D matrices; Q, R cost matrices) alongside Active Inference notation (F, G, mu, pi).
4. Include concrete numerical examples with specific robot parameters to make abstract action concepts tangible.
