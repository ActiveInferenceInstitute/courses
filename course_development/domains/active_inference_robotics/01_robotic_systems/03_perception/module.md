...
A robot arm reaching toward an object whose position is uncertain might first move its wrist camera to a viewpoint that resolves the ambiguity before committing to a grasp trajectory. A mobile robot approaching a T-intersection where its map is uncertain might slow down and sweep its lidar across a wider arc. In both cases, the robot selects actions not to achieve an immediate task goal but to gather information — to reduce the entropy of its posterior beliefs.
...
3. Bajcsy, R., Aloimonos, Y., & Tsotsos, J. K. (2018). Revisiting active perception. *Autonomous Robots*, 42(2), 177-196.
4. Lanillos, P., & Cheng, G. (2018). Adaptive robot body perception and action through active inference. *arXiv preprint arXiv:1806.01671*.
