# Module 07: Communication

> **Quick Navigation**: [Course Home](../README.md) | [Curriculum Home](../../README.md)

## Internal Communication and Culture

Part of **Organizational Systems**.

## Contents

| File | Description |
| --- | --- |
| [module.md](./module.md) | Full lecture (Internal Communication and Culture) |
| [questions.md](./questions.md) | 20 Study Questions |
| [practice_quiz.md](./practice_quiz.md) | Practice Quiz (MC + Short Answer) |
| [lab.md](./lab.md) | Lab: Internal Communication and Culture |
| [dashboard.html](./dashboard.html) | Interactive Dashboard |

## Learning Goals

1. **Map** information flows in an organization
2. **Analyze** culture as a shared generative model
3. **Design** effective team communication

## Resources

- [Notation](../../resources/notation_table.md)
- [Glossary](../../resources/glossary.md)
