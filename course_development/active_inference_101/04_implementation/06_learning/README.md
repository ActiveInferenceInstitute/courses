# Module 06: Learning

> **Quick Navigation**: [Course Home](../README.md) | [Curriculum Home](../../README.md)

## Structure Learning and Model Reduction

Part of **Implementation & Simulation**.

## Contents

| File | Description |
| --- | --- |
| [module.md](./module.md) | Full lecture (Structure Learning and Model Reduction) |
| [questions.md](./questions.md) | 20 Study Questions |
| [practice_quiz.md](./practice_quiz.md) | Practice Quiz (MC + Short Answer) |
| [lab.md](./lab.md) | Lab: Structure Learning and Model Reduction |
| [dashboard.html](./dashboard.html) | Interactive Dashboard |

## Learning Goals

1. **Implement** parameter learning
2. **Perform** Bayesian model comparison
3. **Apply** Occam's principle to model selection

## Resources

- [Notation](../../resources/notation_table.md)
- [Glossary](../../resources/glossary.md)
