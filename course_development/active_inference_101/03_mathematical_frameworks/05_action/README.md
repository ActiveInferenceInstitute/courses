# Module 05: Action

> **Quick Navigation**: [Course Home](../README.md) | [Curriculum Home](../../README.md)

## Active Inference and Expected Free Energy

Part of **Mathematical Frameworks**.

## Contents

| File | Description |
| --- | --- |
| [module.md](./module.md) | Full lecture (Active Inference and Expected Free Energy) |
| [questions.md](./questions.md) | 20 Study Questions |
| [practice_quiz.md](./practice_quiz.md) | Practice Quiz (MC + Short Answer) |
| [lab.md](./lab.md) | Lab: Active Inference and Expected Free Energy |
| [dashboard.html](./dashboard.html) | Interactive Dashboard |

## Learning Goals

1. **Derive** the EFE decomposition
2. **Implement** policy selection
3. **Compare** active inference to reinforcement learning

## Resources

- [Notation](../../resources/notation_table.md)
- [Glossary](../../resources/glossary.md)
