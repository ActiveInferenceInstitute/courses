# Module 03: Perception

> **Quick Navigation**: [Course Home](../README.md) | [Curriculum Home](../../README.md)

## Predictive Processing and Bayesian Inference

Part of **Mathematical Frameworks**.

## Contents

| File | Description |
| --- | --- |
| [module.md](./module.md) | Full lecture (Predictive Processing and Bayesian Inference) |
| [questions.md](./questions.md) | 20 Study Questions |
| [practice_quiz.md](./practice_quiz.md) | Practice Quiz (MC + Short Answer) |
| [lab.md](./lab.md) | Lab: Predictive Processing and Bayesian Inference |
| [dashboard.html](./dashboard.html) | Interactive Dashboard |

## Learning Goals

1. **Derive** Bayesian belief updating
2. **Implement** a simple predictive coding model
3. **Explain** hierarchical message passing

## Resources

- [Notation](../../resources/notation_table.md)
- [Glossary](../../resources/glossary.md)
