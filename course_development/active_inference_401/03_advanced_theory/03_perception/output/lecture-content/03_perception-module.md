...
**The Markov blanket condition**: Internal states are conditionally independent of external states given the blanket states: p(μ | η, s, a) = p(μ | s, a). This statistical separation is what creates the "inside" and "outside" of a system.
...
**The remarkable result**: The flow of internal states (dμ/dt) can be expressed as a gradient descent on variational free energy:

dμ/dt ≈ -∂F/∂μ

where F is the variational free energy of the distribution q(η; μ) that internal states parameterize, with respect to the observations provided by sensory states.
...
**Steady-state density**: At NESS, the system has a time-invariant probability density p(x) where x = (η, s, a, μ), but the dynamics are not at thermodynamic equilibrium — there are circulating (solenoidal) currents.

**Helmholtz decomposition**: The flow of any state can be decomposed into:

dx/dt = (Q - Γ) ∇ ln p(x) + w

where Q is the solenoidal (conservative) component, Γ is the dissipative component, and w is random fluctuation. The dissipative component performs gradient descent on the log-density (free energy minimization); the solenoidal component preserves the steady-state distribution while allowing non-trivial dynamics.
