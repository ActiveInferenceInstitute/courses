...
BF > 3: moderate evidence for m₁. BF > 20: strong evidence. BF > 150: very strong evidence.
...
**The idea**: Given a full model m₀ with posterior q(θ | o, m₀), compute the evidence for a reduced model m₁ (with some parameters fixed or removed) *analytically* from the full model's posterior, without re-running inference.

**The formula**: Under the Laplace approximation:

ln p(o | m₁) ≈ ln p(o | m₀) + ln p(θ*| m₁) - ln p(θ* | m₀) - ½ tr[(Σ₁⁻¹ - Σ₀⁻¹) Σ_posterior]
...
**2. Constraint-based**: Use conditional independence tests to identify the structure. If X ⊥ Y | Z, then there's no direct edge between X and Y given Z. More computationally efficient but sensitive to test reliability.
...
p(number of states) = Dirichlet process
...
- Level 1: p(o | θ, m) — data given parameters
- Level 2: p(θ | φ, m) — parameters given hyperparameters
- Level 3: p(φ | m) — hyperparameters given model

**Empirical Bayes**: Estimate hyperparameters φ by maximizing the marginal likelihood at the second level. This provides a principled method for setting priors — they are learned from the data.
...
**Model space topology**: Define a neighborhood structure on models — model m₁ is a neighbor of m₂ if they differ by one parameter or connection. Structure learning explores this space.
